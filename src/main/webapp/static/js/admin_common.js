/**
 * Created by queric on 2016/5/12 0012.
 */
...
function my_dialog(title, content, btn_obj) {
    ...
    var html = '<div class="modal-dialog modal-sm" style="margin-top:10%;"><div class="modal-content"><div class="modal-header"><button id="close_x_btn" type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h5 class="modal-title" id="myModalLabel">' + title + '</h5></div><div class="modal-body">' + content + '</div><div class="modal-footer">';
    if (btn_obj != false) {
...
function transformFormat(sNodes) {
    var i, l,
    key = 'id',
    parentKey = 'pId',
    childKey = 'children';
    if (!key || key == "" || !sNodes || sNodes.length == 0) return [];
    var r = [];
    var tmpMap = [];
